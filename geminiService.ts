...
export const generateNLSLessonPlan = async (
  ...
  const userPrompt = isEnglishSubject ? `
    ...
    1. SUBJECT & LANGUAGE AUTO-DETECTION (MANDATORY): You MUST analyze the original lesson plan to auto-detect its subject and language. 
       - If the lesson plan is written in English (English subject), you MUST write all NLS additions in ENGLISH using the tags: ===DC_OBJECTIVES=== and ===DC_ACTIVITY_X_POSITION===.
       - If the lesson plan is written in Vietnamese (Other subjects), you MUST write all NLS additions in VIETNAMESE using the tags: ===NLS_MỤC_TIÊU=== and ===NLS_HOẠT_ĐỘNG_X_VỊ_TRÍ===.
    2. MANDATORY INTEGRATION: You MUST find appropriate places to integrate NLS into ALL uploaded lesson plans. Do NOT skip or say "no suitable place".
    ${options.analyzeOnly ? "3. Analyze only, do not edit in detail." : "3. Edit the lesson plan and INTEGRATE DIGITAL COMPETENCE into teaching activities."}
    ${options.detailedReport ? "4. Include a detailed explanation table of selected competence codes at the end." : ""}
  ...
  ` : `
    ...
    1. TỰ ĐỘNG NHẬN DIỆN MÔN HỌC & NGÔN NGỮ (BẮT BUỘC): Bạn PHẢI đọc nội dung giáo án gốc để tự nhận diện ngôn ngữ.
       - Nếu giáo án gốc được viết bằng Tiếng Anh (Môn Tiếng Anh): Bạn BẮT BUỘC phải viết toàn bộ nội dung NLS bổ sung bằng TIẾNG ANH (English). Chú ý sử dụng các thẻ: ===DC_OBJECTIVES=== và ===DC_ACTIVITY_X_POSITION===. Tuyệt đối không dùng tiếng Việt.
       - Nếu giáo án gốc được viết bằng Tiếng Việt (Các môn còn lại): Bạn BẮT BUỘC phải viết toàn bộ nội dung NLS bổ sung bằng TIẾNG VIỆT. Sử dụng các thẻ: ===NLS_MỤC_TIÊU=== và ===NLS_HOẠT_ĐỘNG_X_VỊ_TRÍ===.
    2. BẮT BUỘC TÍCH HỢP: Bắt buộc phải tìm ra vị trí và TÍCH HỢP NLS vào TẤT CẢ các giáo án được tải lên. Tuyệt đối không được bỏ qua hoặc trả lời là "không có chỗ phù hợp".
    ${options.analyzeOnly ? "3. Chỉ phân tích, không chỉnh sửa chi tiết." : "3. Chỉnh sửa giáo án và TÍCH HỢP NĂNG LỰC SỐ vào các hoạt động dạy học."}
    ${options.detailedReport ? "4. Kèm theo bảng giải thích chi tiết mã năng lực đã chọn ở cuối bài." : ""}
